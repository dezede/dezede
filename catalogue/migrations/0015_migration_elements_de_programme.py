...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        ElementDeProgramme = orm['catalogue.ElementDeProgramme']
        empty_els = ElementDeProgramme.objects.filter(evenements=None)
        empty_els.delete()
        Evenement = orm['catalogue.Evenement']
        evenements = Evenement.objects.all()
        for ev in evenements:
            for i, el in enumerate(ev.programme.order_by('classement')):
                if el.evenement is not None:
                    el = deepcopy(el)
                    el.id = None
                el.evenement = ev
                el.position = i
