...
def recreate_relations(Auteur, ContentType, auteur, profession, individu,
                       model_name, relation_name):
    for object in getattr(auteur, relation_name).all():
        content_type = ContentType.objects.get(
            app_label='catalogue', model=model_name)
        Auteur.objects.get_or_create(
            content_type=content_type, object_id=object.id,
            profession=profession, individu=individu)
...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        Auteur = orm['catalogue.Auteur']
        for auteur in Auteur.objects.all():
            profession = auteur.profession
            for individu in auteur.individus.all():
                recreate_relations(Auteur, ContentType, auteur, profession,
                                   individu, 'oeuvre', 'oeuvres')
                recreate_relations(Auteur, ContentType, auteur, profession,
                                   individu, 'source', 'sources')
                recreate_relations(Auteur, ContentType, auteur, profession,
                                   individu, 'document', 'documents')
                recreate_relations(Auteur, ContentType, auteur, profession,
                                   individu, 'illustration', 'illustrations')
            auteur.delete()
