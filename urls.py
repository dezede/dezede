...
urlpatterns = patterns('',
    ...
    url(r'^lieux/id/(?P<pk>\d+)/$', LieuDetailView.as_view(), name='lieu'), # TODO: réserver 'id' lors de la validation
    url(r'^lieux/(?P<slug>[-\w]+)/$', LieuDetailView.as_view(), name='lieu'),
    url(r'^individus/$', IndividuListView.as_view(), name='individus'),
    url(r'^individus/id/(?P<pk>\d+)/$', IndividuDetailView.as_view(), name='individu'), # TODO: réserver 'id' lors de la validation
    url(r'^individus/(?P<slug>[-\w]+)/$', IndividuDetailView.as_view(), name='individu'),
    url(r'^oeuvres/id/(?P<pk>\d+)/$', OeuvreDetailView.as_view(), name='oeuvre'), # TODO: réserver 'id' lors de la validation
    url(r'^oeuvres/(?P<slug>[-\w]+)/$', OeuvreDetailView.as_view(), name='oeuvre'),
    url(r'''^evenements/id/(?P<pk>\d+)/$''', EvenementDetailView.as_view(), name='evenement'), # TODO: réserver 'id' lors de la validation
    url(r'''^evenements'''
        r'''(?:\/(?P<lieu_slug>[-\w]+))?'''
        r'''(?:\/(?P<year>\d+))?'''
        r'''(?:\/(?P<month>\d+))?'''
