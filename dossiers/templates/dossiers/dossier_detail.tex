...
{% block document %}
  ...
  {% for tab in object.kind_tabs %}
    {% if tab.kind == 'evenements' %}
      \addcontentsline{toc}{part}{ {% trans 'Événements' %} }

      {% with evenements=tab.queryset.prefetch_all %}
        ...
        {% endfor %}
      {% endwith %}
    {% elif tab.kind == 'oeuvres' %}
      \addcontentsline{toc}{part}{ {% trans 'Œuvres' %} }

      {% for oeuvre in tab.queryset %}
        {% include 'dossiers/oeuvre_item.tex' %}
      {% endfor %}
    {% elif tab.kind == 'sources' %}
      \addcontentsline{toc}{part}{ {% trans 'Sources' %} }

      {% with sources=tab.queryset.order_by_date %}
        {% for type, type_sources in sources.group_by_type %}
          {% if type_sources|length > 1 %}{% with type_label=type.pluriel|capfirst %}
            \section*{% templatetag openbrace %}{{ type_label }}}
            \addcontentsline{toc}{section}{ {{ type_label }} }
          {% endwith %}{% else %}{% with type_label=type|capfirst %}
            \section*{% templatetag openbrace %}{{ type_label }}}
            \addcontentsline{toc}{section}{ {{ type_label }} }
          {% endwith %}{% endif %}

          {% for source in type_sources %}
            {% include 'dossiers/source_item.tex' %}
          {% endfor %}
        {% endfor %}
      {% endwith %}
    {% endif %}
