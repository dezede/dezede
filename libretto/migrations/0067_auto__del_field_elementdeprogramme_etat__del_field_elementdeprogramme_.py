...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Deleting field 'ElementDeProgramme.etat'
        db.delete_column(u'libretto_elementdeprogramme', 'etat_id')

        # Deleting field 'ElementDeProgramme.notes'
        db.delete_column(u'libretto_elementdeprogramme', 'notes')

        # Removing M2M table for field documents on 'ElementDeProgramme'
        db.delete_table(db.shorten_name(u'libretto_elementdeprogramme_documents'))

        # Removing M2M table for field illustrations on 'ElementDeProgramme'
        db.delete_table(db.shorten_name(u'libretto_elementdeprogramme_illustrations'))


    def backwards(self, orm):

        # User chose to not deal with backwards NULL issues for 'ElementDeProgramme.etat'
        raise RuntimeError("Cannot reverse this migration. 'ElementDeProgramme.etat' and its values cannot be restored.")
        
        # The following code is provided here to aid in writing a correct migration        # Adding field 'ElementDeProgramme.etat'
        db.add_column(u'libretto_elementdeprogramme', 'etat',
                      self.gf('django.db.models.fields.related.ForeignKey')(to=orm['libretto.Etat'], on_delete=models.PROTECT),
                      keep_default=False)

        # Adding field 'ElementDeProgramme.notes'
        db.add_column(u'libretto_elementdeprogramme', 'notes',
                      self.gf('tinymce.models.HTMLField')(default='', blank=True),
                      keep_default=False)

        # Adding M2M table for field documents on 'ElementDeProgramme'
        m2m_table_name = db.shorten_name(u'libretto_elementdeprogramme_documents')
        db.create_table(m2m_table_name, (
            ('id', models.AutoField(verbose_name='ID', primary_key=True, auto_created=True)),
            ('elementdeprogramme', models.ForeignKey(orm[u'libretto.elementdeprogramme'], null=False)),
            ('document', models.ForeignKey(orm[u'libretto.document'], null=False))
        ))
        db.create_unique(m2m_table_name, ['elementdeprogramme_id', 'document_id'])

        # Adding M2M table for field illustrations on 'ElementDeProgramme'
        m2m_table_name = db.shorten_name(u'libretto_elementdeprogramme_illustrations')
        db.create_table(m2m_table_name, (
            ('id', models.AutoField(verbose_name='ID', primary_key=True, auto_created=True)),
            ('elementdeprogramme', models.ForeignKey(orm[u'libretto.elementdeprogramme'], null=False)),
            ('illustration', models.ForeignKey(orm[u'libretto.illustration'], null=False))
        ))
        db.create_unique(m2m_table_name, ['elementdeprogramme_id', 'illustration_id'])
