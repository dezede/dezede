...
class Migration(DataMigration):
    def forwards(self, orm):
        ...
        LogEntry = orm['admin.LogEntry']

        ct1_after = ContentType.objects.get(
            app_label='libretto', model='typedeparentedoeuvrespolymorphic')
        ct2_after = ContentType.objects.get(
            app_label='libretto', model='typedeparentedindividuspolymorphic')

        try:
            ct1 = ContentType.objects.get(app_label='libretto',
                                          model='typedeparentedoeuvres')
        except ContentType.DoesNotExist:
            pass
        else:
            LogEntry.objects.filter(content_type=ct1).update(
                content_type=ct1_after)
            ct1.delete()

        try:
            ct2 = ContentType.objects.get(app_label='libretto',
                                          model='typedeparentedindividus')
        except ContentType.DoesNotExist:
            pass
        else:
            LogEntry.objects.filter(content_type=ct2).update(
                content_type=ct2_after)
            ct2.delete()

        ct1_after.model = 'typedeparentedoeuvres'
        ct1_after.save()
        ct2_after.model = 'typedeparentedindividus'
        ct2_after.save()
    ...
    models = {
        ...
        u'admin.logentry': {
            ...
            'user': ('django.db.models.fields.related.ForeignKey', [], {'to': u"orm['accounts.HierarchicUser']"})
        },
        u'auth.group': {
