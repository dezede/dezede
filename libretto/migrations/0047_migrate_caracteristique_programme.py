...
class Migration(DataMigration):
    def forwards(self, orm):
        CaracteristiqueDElementDeProgramme = orm['libretto.CaracteristiqueDElementDeProgramme']
        PolymorphicCaracteristiqueDElementDeProgramme = orm['libretto.PolymorphicCaracteristiqueDElementDeProgramme']
        ContentType = orm['contenttypes.ContentType']
        ct = ContentType.objects.get_or_create(
            app_label='libretto', model='caracteristiquedelementdeprogramme',
            name=u'caractéristique d’élément de programme')[0]

        if orm['libretto.Caracteristique'].objects.count():
            max_pk = db.execute(
                'select id from libretto_caracteristique order by id desc limit 1;')[0][0]
            db.execute(
                'alter sequence libretto_caracteristique_id_seq restart with %s;',
                [max_pk + 1])

        if orm['libretto.TypeDeCaracteristique'].objects.count():
            max_pk = db.execute(
                'select id from libretto_typedecaracteristique order by id desc limit 1;')[0][0]
            db.execute(
                'alter sequence libretto_typedecaracteristique_id_seq restart with'
                ' %s;',
                [max_pk + 1])

        caracteristiques = CaracteristiqueDElementDeProgramme.objects.order_by('pk')
        for c in caracteristiques:
            new_c = PolymorphicCaracteristiqueDElementDeProgramme.objects.create(
                type=None, valeur=c.nom, classement=c.classement,
                owner=c.owner, polymorphic_ctype=ct,
            )
            elements = list(c.elements_de_programme.all())
            for e in elements:
                e.caracteristiques.remove(c)
            c.delete()
            c.pk = new_c.pk
            c.save()
            for e in elements:
                e.caracteristiques.add(c)
