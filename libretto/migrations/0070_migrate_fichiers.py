...
class Migration(DataMigration):
    def forwards(self, orm):
        TypeDeSource = orm['libretto.TypeDeSource']
        Source = orm['libretto.Source']
        Document = orm['libretto.Document']
        Illustration = orm['libretto.Illustration']
        Fichier = orm['libretto.Fichier']
        Etat = orm['libretto.Etat']
        SourceEvenement = orm['libretto.SourceEvenement']
        SourceOeuvre = orm['libretto.SourceOeuvre']
        SourceIndividu = orm['libretto.SourceIndividu']
        SourceEnsemble = orm['libretto.SourceEnsemble']
        SourceLieu = orm['libretto.SourceLieu']
        SourcePartie = orm['libretto.SourcePartie']

        for source in Source.objects.exclude(illustrations=None,
                                             documents=None):
            for i, obj in enumerate(list(source.illustrations.all())
                                    + list(source.documents.all())):
                is_illustration = isinstance(obj, Illustration)
                file_obj = obj.image if is_illustration else obj.document
                with open(file_obj.path_full) as f:
                    type = 0 if imghdr.what(f) is None else 1
                    Fichier.objects.create(
                        source=source, fichier=File(f), type=type,
                        position=i)

        etat = Etat.objects.get(nom='nouveau')
        archive = TypeDeSource.objects.get(nom__regex='archives?')
        for obj in (list(Illustration.objects.filter(source_set=None))
                    + list(Document.objects.filter(source_set=None))):
            is_illustration = isinstance(obj, Illustration)
            titre = obj.legende if is_illustration else obj.nom
            source = Source.objects.create(type=archive, titre=titre,
                                           etat=etat)

            for linked in obj.evenement_set.all():
                SourceEvenement.objects.create(source=source, evenement=linked)
            for linked in obj.oeuvre_set.all():
                SourceOeuvre.objects.create(source=source, oeuvre=linked)
            for linked in obj.individu_set.all():
                SourceIndividu.objects.create(source=source, individu=linked)
            for linked in obj.ensemble_set.all():
                SourceEnsemble.objects.create(source=source, ensemble=linked)
            for linked in obj.lieu_set.all():
                SourceLieu.objects.create(source=source, lieu=linked)
            for linked in obj.partie_set.all():
                SourcePartie.objects.create(source=source, partie=linked)
