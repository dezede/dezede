...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        Auteur = orm['libretto.Auteur']

        assert tuple(
            Auteur.objects.values_list(
                'content_type__model', flat=True
            ).order_by('content_type__model')
            .distinct()) == ('oeuvre', 'source')

        Auteur.objects.filter(
            content_type__model='oeuvre').update(oeuvre=models.F('object_id'))
        Auteur.objects.filter(
            content_type__model='source').update(source=models.F('object_id'))
