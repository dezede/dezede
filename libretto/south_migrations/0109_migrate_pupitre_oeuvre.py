...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        Pupitre.objects.annotate(n=models.Count('oeuvres')).filter(n=0).delete()
        for pupitre in Pupitre.objects.all():
            oeuvres = list(pupitre.oeuvres.all())
            n = len(oeuvres)
            assert n > 0
            pupitre.oeuvre = oeuvres[0]
            pupitre.save()
            for oeuvre in oeuvres[1:]:
                pupitre.pk = None
                pupitre.oeuvre = oeuvre
                pupitre.save()
