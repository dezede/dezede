...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Adding field 'Role.oeuvre'
        db.add_column(u'libretto_role', 'oeuvre',
                      self.gf('django.db.models.fields.related.ForeignKey')(blank=True, related_name=u'roles', null=True, to=orm['libretto.Oeuvre']),
                      keep_default=False)

        # Adding field 'Ensemble.particule_nom'
        db.add_column(u'libretto_ensemble', 'particule_nom',
                      self.gf('django.db.models.fields.CharField')(db_index=True, default='', max_length=5, blank=True),
                      keep_default=False)

        # Adding index on 'Ensemble', fields ['nom']
        db.create_index(u'libretto_ensemble', ['nom'])


    def backwards(self, orm):
        # Removing index on 'Ensemble', fields ['nom']
        db.delete_index(u'libretto_ensemble', ['nom'])

        # Deleting field 'Role.oeuvre'
        db.delete_column(u'libretto_role', 'oeuvre_id')

        # Deleting field 'Ensemble.particule_nom'
        db.delete_column(u'libretto_ensemble', 'particule_nom')
