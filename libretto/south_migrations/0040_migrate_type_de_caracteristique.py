...
class Migration(DataMigration):
    def forwards(self, orm):
        ...
        PolymorphicTypeDeCaracteristiqueDOeuvre = orm[
            'libretto.PolymorphicTypeDeCaracteristiqueDOeuvre']
        ContentType = orm['contenttypes.ContentType']
        ct = ContentType.objects.get_or_create(
            app_label='libretto', model='typedecaracteristiquedoeuvre',
            name=u'type de caractéristique d’œuvre')[0]

        for t in TypeDeCaracteristiqueDOeuvre.objects.all():
            PolymorphicTypeDeCaracteristiqueDOeuvre.objects.create(
                nom=t.nom, nom_pluriel=t.nom_pluriel, classement=t.classement,
                pk=t.pk, owner=t.owner, polymorphic_ctype=ct,
            )
