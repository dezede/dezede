...
class Migration(DataMigration):
    def forwards(self, orm):
        CaracteristiqueDOeuvre = orm[
            'libretto.CaracteristiqueDOeuvre']
        PolymorphicCaracteristiqueDOeuvre = orm[
            'libretto.PolymorphicCaracteristiqueDOeuvre']
        ContentType = orm['contenttypes.ContentType']
        ct = ContentType.objects.get_or_create(
            app_label='libretto', model='caracteristiquedoeuvre',
            name=u'caractéristique d’œuvre')[0]

        for c in CaracteristiqueDOeuvre.objects.all():
            PolymorphicCaracteristiqueDOeuvre.objects.create(
                type=c.type, valeur=c.valeur, classement=c.classement,
                pk=c.pk, owner=c.owner, polymorphic_ctype=ct,
            )
