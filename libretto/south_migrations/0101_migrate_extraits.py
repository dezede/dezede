...
from common.utils.text import from_roman
...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        TypeDeCaracteristiqueDOeuvre = orm[
            'libretto.TypeDeCaracteristiqueDOeuvre']
        CaracteristiqueDOeuvre = orm['libretto.CaracteristiqueDOeuvre']
        GenreDOeuvre = orm['libretto.GenreDOeuvre']
        Oeuvre = orm['libretto.Oeuvre']

        acte = TypeDeCaracteristiqueDOeuvre.objects.get(nom='acte')
        scene = TypeDeCaracteristiqueDOeuvre.objects.get(nom='scène')
        morceau = TypeDeCaracteristiqueDOeuvre.objects.get(nom='morceau')

        # Transforme les genres du type "3. Allegro"
        genre_extrait_pat = r'^(\d+)\.[   ](.+)$'
        genres_extraits = GenreDOeuvre.objects.filter(
            nom__regex=genre_extrait_pat)
        for genre in genres_extraits:
            numero, tempo = re.match(genre_extrait_pat, genre.nom).groups()
            genre.oeuvres.update(type_extrait=TYPES_EXTRAITS['mouvement'],
                                 numero_extrait=numero, genre=None, tempo=tempo)
        genres_extraits.delete()

        # Transforme les genres du type "1er ordre"
        genre_extrait_pat = r'^(\d+)er?[   ](%s)$' \
                            % '|'.join(TYPES_EXTRAITS.keys())
        genres_extraits = GenreDOeuvre.objects.filter(
            nom__regex=genre_extrait_pat)
        for genre in genres_extraits:
            numero, type_extrait = re.match(genre_extrait_pat,
                                            genre.nom).groups()
            type_extrait = TYPES_EXTRAITS[type_extrait]
            assert not genre.oeuvres.exclude(type_extrait=None).exists()
            genre.oeuvres.update(
                type_extrait=type_extrait, numero_extrait=numero, genre=None)
        genres_extraits.delete()

        # Transforme les genres du type "scène n° 5", "acte II", etc
        genre_extrait_pat = r'^(%s)[   ](?:n°[   ]?)?(\d+|[IV]+)$' \
                            % '|'.join(TYPES_EXTRAITS.keys())
        genres_extraits = GenreDOeuvre.objects.filter(
            nom__regex=genre_extrait_pat)
        for genre in genres_extraits:
            type_extrait, numero = re.match(genre_extrait_pat,
                                            genre.nom).groups()
            type_extrait = TYPES_EXTRAITS[type_extrait]
            if 'I' in numero or 'V' in numero:
                numero = str(from_roman(numero))
            assert not genre.oeuvres.exclude(type_extrait=None).exists()
            genre.oeuvres.update(
                type_extrait=type_extrait, numero_extrait=numero, genre=None)
        genres_extraits.delete()

        # Transforme les caractéristiques de morceau
        morceau_pat = r'^[Nn]°[   ](\d+)$'
        caracteristiques_morceau = CaracteristiqueDOeuvre.objects.filter(
            type=morceau, valeur__regex=morceau_pat)
        for caracteristique in caracteristiques_morceau:
            numero = re.match(morceau_pat, caracteristique.valeur).group(1)
            assert not caracteristique.oeuvres.exclude(type_extrait=None).exists()
            caracteristique.oeuvres.update(
                type_extrait=TYPES_EXTRAITS['morceau'], numero_extrait=numero)
        caracteristiques_morceau.delete()

        # Transforme les caractéristiques de scène
        scene_pat = r'^scène[   ](?:: )?(\d+)$'
        caracteristiques_scene = CaracteristiqueDOeuvre.objects.filter(
            type=scene, valeur__regex=scene_pat)
        for caracteristique in caracteristiques_scene:
            numero = re.match(scene_pat, caracteristique.valeur).group(1)
            assert not caracteristique.oeuvres.exclude(type_extrait=None).exists()
            caracteristique.oeuvres.update(
                type_extrait=TYPES_EXTRAITS['scène'], numero_extrait=numero)
        caracteristiques_scene.delete()

        # Transforme les caractéristiques d’acte
        caracteristiques_acte = CaracteristiqueDOeuvre.objects.filter(
            type=acte)
        NUMEROS = {'I': 1, 'II': 2, 'III': 3, 'IV': 4, 'V': 5}
        for caracteristique in caracteristiques_acte:
            numero = re.match(r'^acte(?:[   ]:)? (I|II|III|IV|V)$',
                              caracteristique.valeur).group(1)
            numero = NUMEROS[numero]
            assert not caracteristique.oeuvres.exclude(
                contenu_dans__type_extrait=None).exists()
            for oeuvre in caracteristique.oeuvres.all():
                assert oeuvre.contenu_dans_id is not None
                oeuvre_acte = Oeuvre.objects.get_or_create(
                    contenu_dans_id=oeuvre.contenu_dans_id,
                    type_extrait=TYPES_EXTRAITS['acte'],
                    numero_extrait=numero,
                    defaults=dict(
                        slug='%d-acte-%s' % (oeuvre.contenu_dans_id, numero),
                        etat_id=19, lft=0, rght=0, tree_id=0, level=0,
                    ),
                )[0]
                oeuvre.contenu_dans = oeuvre_acte
                oeuvre.caracteristiques.remove(caracteristique)
                oeuvre.save()
        caracteristiques_acte.delete()
        acte.delete()

        print('Lance `Oeuvre.objects.rebuild()` manuellement!')
    ...
    models = {
        ...
        u'libretto.elementdedistribution': {
            'Meta': {'ordering': "(u'pupitre', u'profession', u'individu', u'ensemble')", 'object_name': 'ElementDeDistribution'},
