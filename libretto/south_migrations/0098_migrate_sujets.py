...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        sujet = TypeDeCaracteristique.objects.get(pk=19)
        caracteristiques = CaracteristiqueDOeuvre.objects.filter(type=sujet)

        assert not caracteristiques.exclude(valeur__startswith='sur ').exists()

        for caracteristique in caracteristiques:
            for oeuvre in caracteristique.oeuvres.all():
                assert caracteristiques.filter(oeuvres=oeuvre).count() == 1
                oeuvre.sujet = caracteristique.valeur[4:]
                oeuvre.save()
