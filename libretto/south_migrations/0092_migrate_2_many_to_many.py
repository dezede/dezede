...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        assert not ElementDeDistribution.objects.filter(
            individus__isnull=False, ensembles__isnull=False).exists()

        for ed in ElementDeDistribution.objects.exclude(individus=None):
            elements_de_programme = list(ed.elements_de_programme.all())
            for individu in ed.individus.all():
                ed.individu = individu
                ed.save()
                ed.elements_de_programme.add(*elements_de_programme)
                ed.pk = None

        for ed in ElementDeDistribution.objects.exclude(ensembles=None):
            elements_de_programme = list(ed.elements_de_programme.all())
            for ensemble in ed.ensembles.all():
                ed.ensemble = ensemble
                ed.save()
                ed.elements_de_programme.add(*elements_de_programme)
                ed.pk = None

        ElementDeDistribution.objects.filter(
            pupitre__isnull=False, profession__isnull=False
        ).update(profession=None)
