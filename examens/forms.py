...
from django.forms import Form, CharField, ModelChoiceField, HiddenInput, \
    ChoiceField
...
LEVELS = [level for level, pk_list in LEVELS_DATA]
SOURCE_IDS = [pk for level, pk_list in LEVELS_DATA for pk in pk_list]
...
class SourceExamenForm(Form):
    level = ChoiceField(choices=zip(LEVELS, LEVELS), widget=HiddenInput)
    source = ModelChoiceField(
        Source.objects.filter(id__in=SOURCE_IDS), widget=HiddenInput)
    transcription = CharField(widget=TinyMCE, required=False)
